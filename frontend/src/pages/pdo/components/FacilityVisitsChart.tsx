...
import { useFacilityStatusCount } from '../../../hooks/PDOHooks/useFacilityVisits';
...
/**
 * Build a "YYYY-MM-DD" string from parts to avoid UTC offset shifting.
 */
...
export const FacilityVisitsChart: React.FC<FacilityVisitsChartProps> = ({
  ...
  // ─── Derive date range from selected month + year ─────────────────────────
  ...
  // ─── Query — auto-refreshes every 30 s ───────────────────────────────────
  const {
    data: statusData = { active: 0, inactive: 0, closed: 0 },
    isLoading: loading,
  } = useFacilityStatusCount(dateFrom, dateTo, province);
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 transition-colors h-full flex flex-col">
          ...
          <div>
            ...
            {selectedProvince !== 'All Provinces' && (
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                Province:{' '}
                <span className="font-medium text-gray-700 dark:text-gray-300">{selectedProvince}</span>
              </p>
            )}
          </div>
        ...
        <div id="facility-visits-chart" className="flex-1 flex items-center justify-center bg-gray-50 dark:bg-gray-800/50 rounded-lg min-h-[300px] p-6">
          {loading ? (
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-500 mt-2">{selectedProvince} — {selectedMonth} {selectedYear}</p>
            </div>
          ) : (
              ...
              <div className="grid grid-cols-3 gap-6 max-w-2xl mx-auto">
                ...
                <div className="text-center">
                  <div className="relative w-32 h-32 mx-auto mb-3">
                    <svg viewBox="0 0 100 100" className="transform -rotate-90 w-full h-full">
                      ...
                      <circle cx="50" cy="50" r="40" fill="none" stroke="#10b981" strokeWidth="20" strokeDasharray={`${(statusData.active / total) * circleLength} ${circleLength}`} className="transition-all duration-500" />
                    </svg>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-2xl font-bold text-green-700 dark:text-green-300">{statusData.active}</div>
                    </div>
                  </div>
                ...
                <div className="text-center">
                  <div className="relative w-32 h-32 mx-auto mb-3">
                    <svg viewBox="0 0 100 100" className="transform -rotate-90 w-full h-full">
                      <circle cx="50" cy="50" r="40" fill="none" stroke="#e5e7eb" strokeWidth="20" />
                      <circle cx="50" cy="50" r="40" fill="none" stroke="#eab308" strokeWidth="20" strokeDasharray={`${(statusData.inactive / total) * circleLength} ${circleLength}`} className="transition-all duration-500" />
                    </svg>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-2xl font-bold text-yellow-700 dark:text-yellow-300">{statusData.inactive}</div>
                    </div>
                  </div>
                ...
                <div className="text-center">
                  <div className="relative w-32 h-32 mx-auto mb-3">
                    <svg viewBox="0 0 100 100" className="transform -rotate-90 w-full h-full">
                      <circle cx="50" cy="50" r="40" fill="none" stroke="#e5e7eb" strokeWidth="20" />
                      <circle cx="50" cy="50" r="40" fill="none" stroke="#6b7280" strokeWidth="20" strokeDasharray={`${(statusData.closed / total) * circleLength} ${circleLength}`} className="transition-all duration-500" />
                    </svg>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-2xl font-bold text-gray-700 dark:text-gray-300">{statusData.closed}</div>
                    </div>
                  </div>
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-800 flex gap-2 flex-wrap">
        <select value={selectedProvince} onChange={(e) => onProvinceChange(e.target.value)} className="px-3 py-2 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg text-sm border-0 focus:ring-2 focus:ring-blue-500">
          {PROVINCES.map((p) => <option key={p} value={p}>{p}</option>)}
        </select>
        <select value={selectedYear} onChange={(e) => onYearChange(e.target.value)} className="px-3 py-2 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg text-sm border-0 focus:ring-2 focus:ring-blue-500">
          {YEARS.map((y) => <option key={y} value={y}>{y}</option>)}
        </select>
        <select value={selectedMonth} onChange={(e) => onMonthChange(e.target.value)} className="px-3 py-2 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg text-sm border-0 focus:ring-2 focus:ring-blue-500">
