...
export const CumulativePerProvince: React.FC<Props> = ({
  ...
  const handleDownload = async (format: 'png' | 'svg' | 'excel') => {
    ...
    const chartTitle = chartType === "single-province" 
      ? `Cumulative_${province}_${monthRange}_${yearA}_vs_${yearB}`
      : `Cumulative_All_Provinces_${monthRange}_${yearA}_vs_${yearB}`;
    ...
    try {
      if (format === 'excel') {
        ...
        await downloadChart({
          elementId: 'cumulative-per-province-chart',
          filename,
          format: 'excel',
          data: excelData,
          sheetName: 'Cumulative Data'
        });
      } else {
        await downloadChart({
          elementId: 'cumulative-per-province-chart',
  ...
  return (
    ...
    >
      ...
      {/* Chart */}
      <div id="cumulative-per-province-chart" className="flex-1 p-5">
        {loading ? (
