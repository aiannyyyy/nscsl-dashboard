...
import { useLogbookMnemonicStats } from '../../../hooks/LaboratoryHooks/useLogbookEndorsement';
...
// ─── Component ────────────────────────────────────────────────────────────────
export const EndorsementMnemonicChart: React.FC<Props> = ({ expanded, onExpand }) => {
  const { data, isLoading } = useLogbookMnemonicStats();
  const chartData = (data?.data || []).map((item) => ({
    mnemonic: item.mnemonic || 'N/A',
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            <CartesianGrid
              strokeDasharray="3 3"
              stroke="#e5e7eb"
              className="dark:[&>line]:stroke-gray-700"
              vertical={false}
            />
            <XAxis
              dataKey="mnemonic"
