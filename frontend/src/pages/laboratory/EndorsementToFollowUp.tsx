...
export const EndorsementToFollowUp: React.FC = () => {
  ...
  const { data: endorsementListData } = useLogbookEndorsementList();
  ...
  useEffect(() => {
    if (!endorsementIdParam || !endorsementListData?.data?.length) return;
    const id = Number(endorsementIdParam);
    if (!Number.isFinite(id)) return;
    const row = endorsementListData.data.find((r) => r.id === id);
    if (!row?.date_input) return;
    const y = row.date_input.trim().slice(0, 10);
    if (y) setSelectedDate(y);
  }, [endorsementIdParam, endorsementListData?.data]);
