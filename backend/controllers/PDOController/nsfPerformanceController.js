...
async function generateCrystalReportPDF(submid, dateFrom, dateTo, facilityName = '') {
    try {
        ...
        console.log('📄 Crystal Report Configuration:');
        ...
        console.log('🚀 Executing Crystal Reports VB.NET Generator...');
        ...
        if (stdout) {
            console.log('📋 VB.NET Crystal Reports Output:');
            stdout.split('\n').forEach(line => {
        ...
        if (stderr) {
            console.warn('⚠️ VB.NET Warnings/Errors:');
            stderr.split('\n').forEach(line => {
        ...
        console.log(`✅ Crystal Report generated by VB.NET: ${stats.size} bytes`);

        // Copy to final output directory
        if (reportOutputPath !== finalOutputPath) {
            fs.copyFileSync(reportOutputPath, finalOutputPath);
            console.log(`📋 Copied to final location: ${finalOutputPath}`);
        }

        return { filePath: finalOutputPath, fileName };

    } catch (error) {
        console.error('❌ Error executing Crystal Reports VB.NET generator:', error);
...
exports.generateNsfReport = async (req, res) => {
    ...
    try {
        ...
        fileStream.on('end', () => {
            console.log(`[${requestId}] ✅ PDF streamed successfully`);
...
exports.serveReport = async (req, res) => {
    ...
    try {
        ...
        console.log(`[${requestId}] ✅ Report served successfully`);
 ...
 * ⚠️ UPDATED: Removed Edge.js references, now reports VB.NET executable status
 */
exports.reportSystemHealth = async (req, res) => {
    try {
        ...
        try {
            ...
            console.error('⚠️ Directory check error:', dirError.message);
        }
        ...
        try {
            ...
            console.error('⚠️ Files check error:', filesError.message);
        }
        
        console.log('🔧 VB.NET Executable Status:', exeExists ? 'Available' : 'Not Available');
        ...
        // ⚠️ KEY CHANGE: Use "vbnet_executable" instead of "edgeJsIntegration"
        const health = {
            status: status,
        ...
        console.log('✅ Health Check Result:', { status, issuesCount: issues.length });
